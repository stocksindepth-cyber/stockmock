...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#080C16]/80 backdrop-blur-xl border-b border-white/5">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-3 shrink-0 mr-4">
            <div className="relative w-8 h-8 rounded-md overflow-hidden border border-white/10 flex items-center justify-center bg-transparent">
              <Image src="/logo.png" alt="OptionsGyani Logo" width={32} height={32} className="object-cover" />
            </div>
            <span className="text-xl font-bold tracking-tight text-white hidden sm:block">OptionsGyani</span>
          </Link>

          {/* Desktop nav links - Hidden below XL to prevent squeezing */}
          <div className="hidden xl:flex items-center justify-center space-x-1 flex-1 px-4">
          ...
          {/* Right section: Auth Cluster + Mobile Toggle */}
          <div className="flex items-center justify-end gap-3 md:gap-4 shrink-0">
            {/* Auth Cluster */}
            {loading ? null : currentUser ? (
              <div className="flex items-center gap-3 md:gap-4">
                {userProfile?.plan === "free" && (
                  <Link href="/pricing" className="hidden sm:flex group items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/20 text-amber-500 text-xs font-bold transition-all hover:border-amber-500/40" title="Daily Free Limit">
                    <Zap className="w-3.5 h-3.5" />
                    <span>{userProfile.simulationsRunToday || 0} / {userProfile.simulationsLimit || 5}</span>
                  </Link>
                )}
                ...
                <div className="relative group/profile">
                  <button className="flex items-center gap-2.5 hover:bg-white/5 rounded-full pl-1.5 pr-2 md:pr-4 py-1 transition-colors border border-transparent hover:border-white/10">
                    ...
                    <div className="hidden md:flex flex-col items-start leading-none">
                      <span className="text-sm font-medium text-white max-w-[100px] truncate">{currentUser.displayName || currentUser.email.split('@')[0]}</span>
                  ...
                  <div className="absolute right-0 top-full pt-2 opacity-0 invisible group-hover/profile:opacity-100 group-hover/profile:visible transition-all duration-200 translate-y-2 group-hover/profile:translate-y-0">
                    <div className="w-56 glass-card rounded-xl border border-white/10 shadow-xl overflow-hidden flex flex-col p-1 bg-[#101726]/95 backdrop-blur-xl">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <Link href="/login" className="px-3 md:px-4 py-2 text-sm font-semibold text-slate-300 hover:text-white transition-colors">Log In</Link>
                <Link href="/signup" className="px-3 md:px-4 py-2 text-sm font-semibold bg-white text-slate-900 rounded-lg hover:bg-slate-200 transition-colors">Sign Up</Link>
              </div>
            )}
            ...
            <button
              ...
              className="xl:hidden p-2 text-slate-400 hover:text-white ml-2"
            >
      ...
      {/* Mobile nav dropdown */}
      {mobileOpen && (
        <div className="xl:hidden border-t border-white/5 bg-[#0B0F19] px-4 py-3 space-y-1 max-h-[80vh] overflow-y-auto">
          {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
            const isActive = pathname === href || pathname?.startsWith(href + "/");
            return (
              <Link
